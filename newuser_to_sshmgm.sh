...
mysql --login-path=remote --host=ssh_mgm keys-db << EOF#!/bin/bash
...
mysql --login-path=remote --host=ssh_mgm keys-db << EOF
	INSERT INTO stage1_users (user_account, user_group, user_pubkey, user_prikey, user_create) \
	VALUES ("$user_account", "$user_group", "$stage1_user_pubkey", "$stage1_user_prikey", NOW()); \
	INSERT INTO stage2_users (user_account, user_group, user_pubkey, user_prikey, user_create) \
	VALUES ("$user_account", "$user_group", "$stage2_user_pubkey", "$stage2_user_prikey", NOW()); 
EOF
